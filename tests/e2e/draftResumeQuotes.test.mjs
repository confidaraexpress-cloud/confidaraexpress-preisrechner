...
test("Provider-Neutralitaet: Fortsetzen mit Antwort OHNE JUMiNGO-Referenz zeigt Angebote", async () => {
  ...
  await page.route("**/api/jumingo/calculate-price", async (route) => {
    ...
    return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({
      ...
      tariffs: [{ offerId: "0123456789abcdef0123456789abcdef", publicCarrierId: "dhl",
                  ...
                  transitDaysMax: 1, bookable: true, unavailableReason: null, requiredPriceInputs: [] }],
      availableShippingModes: [], publicCarriers: [{ id: "dhl", name: "DHL Express" }],
      customsRequired: false, fromCountryCode: "DE", toCountryCode: "DE", exportDeclaration: null,
  ...
  await page.waitForSelector(".offer-card", { timeout: 20_000 });
  assert.equal(await page.getByText(/keine verlässliche Sendungsgrundlage/).count(), 0,
    "eine Antwort ohne JUMiNGO-Referenz wurde als Persistenzfehler blockiert");
  assert.equal(await page.locator(".offer-card").count(), 1);
  assert.equal(calls.length, 1);
  assert.equal(calls[0].sourceFormDraftId, 77);
});
...
test("Provider-Neutralitaet: ohne lokale Sendung bleibt der Fortsetzen-Guard scharf", async () => {
  ...
  await ctaOf(page).click();
  await page.waitForTimeout(1500);
  assert.equal(await page.locator(".offer-card").count(), 0, "ohne lokale Sendung duerfen keine Angebote erscheinen");
  assert.match(await page.locator(".alert-error").first().textContent(), /keine verlässliche Sendungsgrundlage/);
});
