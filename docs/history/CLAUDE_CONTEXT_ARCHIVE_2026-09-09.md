> # HISTORICAL / NON-CANONICAL — DO NOT USE AS CURRENT PROJECT STATE
>
> This file records historical implementation context only.
> It must not be used as current ConfidaraExpress architecture, product state or source of truth.
> Current project-wide context is defined by `CONFIDARAEXPRESS_CANONICAL_CONTEXT.md`.
>
> Es ist die vollstaendige Frontend-`CLAUDE.md` mit Stand 2026-09-09, bevor sie auf
> repository-spezifische Arbeitsregeln reduziert wurde. Sie wird **nicht** automatisch als
> Claude-Kontext geladen und ist bewusst unveraendert erhalten: die Begruendungen hinter
> vielen Designentscheidungen (gemessene Befunde, verworfene Alternativen, konkrete Fallen)
> sind weiterhin wertvoll, ihre Zustandsaussagen aber nicht mehr zwingend aktuell.
>
> **Bekannt veraltet in diesem Archiv** (Stand der Ablage):
> * die Angabe „React 18" — `package.json` fuehrt React 19
> * der Abschnitt „Aktiver Feature-Branch" — der genannte Branch ist laengst gemergt
> * konkrete Pruefungsanzahlen und einmalige Auditbefunde
> * die Beschreibung der Adressart-Bedienoberflaeche der Buchungsseite — seit TG-7 sind
>   bereits beantwortete Angaben dort read-only
>
> Fuer den aktuellen Zustand gilt ausschliesslich: menschliche Produktentscheidung →
> `origin/main` → Tests/CI → `CONFIDARAEXPRESS_CANONICAL_CONTEXT.md` → `CLAUDE.md`.
