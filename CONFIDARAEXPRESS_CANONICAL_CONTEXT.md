...
- 6.4: Der Druckvertrag als providerneutraler Vertrag: neue Spalte `shipment_provider_documents.page_count` (additiv, idempotent; `NULL` = „nicht erfasst“ und zählt wie mehrseitig), Seitenzahl gelesen mit `pdf-lib` (ein synchroner Byte-Scan scheitert nachweislich an PDF 1.5 mit komprimierten Objektströmen), ein Kundenhinweis für jeden nicht nachweislich einseitigen Beleg. Keine Seitenklassifikation, keine Seitennummer im Text, keine ServiceID-Weiche, keine Carrierunterscheidung.
