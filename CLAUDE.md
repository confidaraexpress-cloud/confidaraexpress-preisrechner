...
- **Ohne Zuschlag (DHL Foundation):** nennt das Angebot die Angabe zusätzlich in `surchargeFreePriceInputs` (`offerResidentialSurchargeFree`), lautet der Kartenhinweis „Die Art der Lieferadresse wird vor der Buchung abgefragt.", beide Karten zeigen „+ 0,00 €", der Abschnitt lädt mit „Preis wird geprüft …" und meldet einen vorübergehenden Fehler als „Der Preis konnte nicht geprüft werden.". Optionen mit Zuschlag oder zwei verschiedenen Preisen und eine Bindung mit „Zuschlag Privatadresse" werden verworfen. Ohne das Feld gilt der bisherige Vertrag; entschieden wird nur am Feld, nie an Service oder Carrier. Die Neubepreisung der Absicherung wartet 60 s (`repriceInsurance`).
...
- **UPS · Standardversand Mehrpaket (freigegeben, TG26):** auswählbar wie der Standardversand — „Vorläufiger Preis", Zuschlagshinweis, Art der Lieferadresse auf der Buchungsseite, danach Absicherung und Buchung, alles über die Verträge oben. Ohne Profil und ohne Prognose bleibt der bisherige Detailbereich: Laufzeit „1–5 Tage", Sendungsverfolgung, Drucker und die Einschränkung „max. 2 Packstücke" kommen aus den Serverfeldern — keine Zahl im Code. Eine Abholung heute gibt es nicht (der Server sperrt den Tag). Solange die Art der Lieferadresse aussteht, sagt der Server nichts über die Absicherung; der Detailbereich behauptet dann auch kein „Keine Zusatzversicherung verfügbar." (`offerCardInsurance`).
