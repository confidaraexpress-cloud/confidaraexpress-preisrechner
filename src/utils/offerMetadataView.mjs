...
import { labelFileFormatName, labelSizeName } from "./labelFormatOptions.mjs";
...
/* Eine Liste von Kundennamen — uebersetzt, entdoppelt, ohne unbekannte Rohwerte.
   TG22 Golden Offer Contract: die Namen kommen aus DEMSELBEN Uebersetzer wie der Lieferhinweis
   der Buchung (utils/labelFormatOptions.mjs). Bis hierher stand auf der Karte die rohe
   Serverschreibweise („A4 / Thermal"), in der Buchung dagegen „DIN A4 und Thermodruck". Ein
   Wert ohne Kundennamen erscheint nicht. */
const kundennamen = (w, uebersetze) => {
  const liste = [];
  for (const wert of Array.isArray(w) ? w : []) {
    const name = uebersetze(wert);
    if (name && !liste.includes(name)) liste.push(name);
  }
  return liste;
};
 ...
 *
 * „PDF · DIN A4 / Thermodruck" — Formate und Groessen durch einen Mittelpunkt getrennt, die
 * Werte innerhalb einer Gruppe durch Schraegstrich. Fehlt eine der beiden Gruppen, entsteht die
...
export function labelCapabilityLine(tarif) {
  ...
  const formate = kundennamen(t.labelFormats, labelFileFormatName);
  const groessen = kundennamen(t.labelSizes, labelSizeName);
  const teile = [];
