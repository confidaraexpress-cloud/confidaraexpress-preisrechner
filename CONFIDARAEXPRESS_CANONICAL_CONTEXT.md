...
Transglobal-Angebote dürfen Kunden sichtbar sein. Außer dem Referenzservice 22 und dem Expressversand (Service 23, siehe unten) sind sie quote_only und nicht buchbar — das gilt auch für UPS Express (Service 29) und UPS Standard Multi (Service 26), bis ihre Evidenz vorliegt (siehe unten).
...
- **UPS-Inventar** (bestätigt 2026-09-16): Transglobal führt für UPS genau die Services 22, 23, 26 und 29. **Service 22: DONE** (Referenzservice). **Service 23: DONE** (Staging-Buchungssmoke bestanden).
- **Service 29 (UPS Express, kundenseitig „UPS · Express") — Implementierung vollständig, Buchungsfreigabe evidenzgebunden** (bestätigt 2026-09-16): dieselbe Fähigkeitsstruktur wie Service 23 (Preisklasse EXPRESS mit dem Expressaufschlag des Kontos, Art der Lieferadresse nach der Auswahl, Abholung am selben Tag, Transportabsicherung nach Warenwert, Tracking, Label „PDF · DIN A4 / Thermodruck", Kurzbeschreibung „Schneller Expressversand für eilige Sendungen.", voraussichtliche Lieferung als Werktagsprognose ohne Uhrzeit), aber ausschließlich mit belegten Werten: höchstens ein Packstück als CE-Grenze; kein Höchstgewicht, kein Volumendivisor, keine Länge, kein Gurtmaß, keine Zeitzusage. Der Name „Express" ist neutral und innerhalb von UPS eindeutig (neben „Expressversand" der 23), ohne „10:30", „12:00", „vormittags" oder „garantiert", und entspricht dem CE-Tarifnamen des JUMiNGO-Pendants „EXPRESS ®". Service 29 bleibt quote_only, bis ein gezielter Staging-Buchungssmoke bestanden ist; die Freigabe ist danach allein der Wechsel `publicBookable`/`publicBookableScopes` (DE→DE) im Policy-Eintrag.
- **Service 26 (UPS Standard Multi, kundenseitig „UPS · Standardversand Mehrpaket") — Mehrpaketvertrag vollständig, Anbieterevidenz ausstehend** (bestätigt 2026-09-16): N gleiche Packstücke nach dem CE-Modell „Identische Pakete" in stabiler Reihenfolge; Angebotsidentität und Revalidierung binden Stückzahl, Maße und Gewicht und sperren bei jeder Abweichung; die Buchung ist für mehrere Packstücke vorbereitet (mehrere Sendungsnummern und Etappen, Labels „Versandlabel 1 von N (A4)" usw., nur echte Anbieterlabels, keine Dubletten). Art der Lieferadresse, Abholung am selben Tag, Transportabsicherung, Produktprofil und Prognose bleiben bis zur Evidenz aus; es gibt keine erfundene Grenze (eine CE-Grenze wäre als solche zu benennen). Service 26 bleibt quote_only bis zu einem Staging-Quote mit zwei echten Packstücken und genau einer danach freigegebenen Staging-Buchung.
- **Transportabsicherung ist eine kuratierte Fähigkeit** (bestätigt 2026-09-16): das Absicherungsextra des Anbieters allein genügt nicht — angeboten, bepreist und gebucht wird sie nur für Services, deren Eintrag sie trägt (22, 23, 29).
...
- Service `publicBookable` **und** Routenbereich in `publicBookableScopes` (heute die Services 22 und 23, nur DE→DE),
...
- Full Quote mit Buchungsreferenz, kuratierte Übergabeart und Preisklasse, gebundene Art der Lieferadresse (Services 22 und 23: nach der Angebotsauswahl; ohne Bindung nicht buchbar),
...
UPS-Vervollständigung (wirksam nach Merge): Service 29 (UPS Express, „Express") trägt die Kuratierung der 23 mit ausschließlich belegten Werten — `maxPackages: 1` als CE-Grenze, kein Gewicht, Produktprofil `express_urgent` mit nicht belegtem Divisor (`volumetricDivisor: null`: Profil ja, Volumengewichtsformel nein), Paletten und Koffer, Tracking, Druckpflicht, Absicherung, Art der Lieferadresse, Abholung am selben Tag und Werktagsprognose — aber `publicBookable: false`: bis zur Freigabe Preisauskunft ohne Adressfrage, ohne Abholung heute und ohne Absicherung; Optionen, Bindung, Neubepreisung und `/book` enden vor jedem Providerkontakt. Belegt ist ein Staging-Full-Quote (2026-09-16: Fracht 24,36, Privatzuschlag 2,65, Same-Day-Gebühr 2,52 mit Abholschluss 17:00, Absicherung 10,00 bei Warenwert 500, Labels A4 und Thermal); die Freigabe ist nach einem bestandenen Staging-Buchungssmoke nur der Wechsel der beiden Freigabefelder, und ein Test beweist die ganze Kette mit genau diesem Wechsel. Service 26 (UPS Standard Multi, „Standardversand Mehrpaket") ist nur DE→DE, ohne kuratierte Fähigkeiten, ohne Profil, ohne Prognose und ohne erfundene Grenze; das Full-Quote-Paketmodell fragt N gleiche Packstücke an, Revalidierung und Buchung lesen dieselbe Entwurfszeile, und mehrere Labels und Sendungsnummern werden zu „Versandlabel k von N (Format)" und je einer Tracking-Etappe. Die Transportabsicherung entsteht nur für Services mit kuratierter Fähigkeit (`capabilities.transportInsurance`: 22, 23, 29) — das Anbieterextra allein genügt nicht. Jede ausgewertete Transglobal-Antwort schreibt eine Bestandszeile (`[TG service-inventory]`: angebotene ServiceIDs, abgelehnte nur mit ServiceID, Carrier und Grund, unbekannte ServiceIDs kuratierter Carrier als Warnung) — ohne zusätzlichen Request. Nachträgliche Anbieterbelastungen bleiben Befund der bestehenden Rechnungsprüfung; die Stornierung ist unverändert. Kein Schalter, keine ENV-Änderung, keine Schemaänderung; JUMiNGO funktional unverändert (das kuratierte Paar „EXPRESS ®" ↔ 29 bestand bereits; für die 26 gibt es kein Paar).
...
| Transglobal Booking | IMPLEMENTED_CONDITIONALLY | default aus; öffentlich buchbar vorbereitet nur Service 22 und Service 23 (TG23, Backend gemergt, Staging-Smoke bestanden), DE→DE hinter Produktschalter, Buchungsschalter und Kontowährung; Art der Lieferadresse nach der Angebotsauswahl (TG22 Residential, wirksam nach Merge); Abholung am selben Tag bis zum wirksamen Abholschluss (TG22 Same-Day, wirksam nach Merge); Produktprofil und 70-kg-Grenze je Packstück (TG22 Package A, wirksam nach Merge); voraussichtliche Lieferung als CE-Prognose aus Abholtag und Laufzeit (TG22 Package B, wirksam nach Merge); UPS Express (29) implementiert und UPS Standard Multi (26) mit Mehrpaketvertrag, beide quote_only bis zu ihrer Evidenz; Transportabsicherung nur für kuratierte Services; Bestandsdiagnose je Quote (UPS-Vervollständigung, wirksam nach Merge) |
...
UPS-Vervollständigung (wirksam nach Merge): „UPS · Express" erscheint bis zur Freigabe als vollwertige Preisauskunft — Preis sichtbar, „Derzeit nicht direkt buchbar", keine Adressfrage, keine Abholung heute, keine Absicherung, keine Auszeichnung — mit den fünf Detailabschnitten, aber ohne Volumengewichtsformel, ohne Abrechnungshinweis und ohne Gewichtszeile; „Voraussichtliche Lieferung" als einzelner Tag. Nach der Freigabe genügen dieselben Felder wie beim Expressversand, die Oberfläche ändert sich nicht. „UPS · Standardversand Mehrpaket" erscheint als Preisauskunft mit dem bisherigen Detailbereich (kein Profil, keine Prognose, keine erfundene Grenze). Mehrere Packstücke heißen überall „2 Pakete · je 4 kg · 40 × 30 × 20 cm"; mehrere Versandbelege tragen die Servernamen „Versandlabel 1 von 2 (A4)" usw. in Serverreihenfolge, das Abholetikett zuletzt; mehrere Trackingnummern stehen in der Liste als „2 Trackingnummern", im Detail vollständig und je Etappe. Gesperrt ist bei einer Preisauskunft allein der Auswahlknopf; die Karte trägt kein `aria-disabled`, „Details anzeigen" bleibt bedienbar — auch für Screenreader. Kein Anbietername, keine ServiceID-Prüfung, keine Produktnamen im Frontendcode.
