...
// `offerId`: das Stufenmodell (Standard/Premium) läuft dort unverändert durch den
// bisherigen Handler, das Deckungsbetragsmodell bindet Preis und Auswahl an das
// Angebot. WICHTIG (Backend-Vertrag): es werden KEINE Client-Preise übertragen; im
// Stufenmodell bleibt `insuranceType` FLACH. Gibt die rohe Response zurück (der
// Aufrufer wertet Status/JSON selbst aus, konsistent mit den übrigen Callern).
// Optionales AbortSignal, um veraltete In-Flight-Requests abzubrechen.
export function repriceInsurance(payload, { signal } = {}) {
  return apiFetch(`/api/insurance/reprice`, {
    method: "POST",
    auth: true,
