...
- **Karte:** nur mit `pickupToday: true`, `pickupTodayUntil` („HH:MM") **und** beiden Beträgen `sameDaySurchargeNet`/`sameDaySurchargeGross` erscheinen „Zuschlag für Abholung am selben Tag: +X,XX €" (netto oder brutto wie der Kartenpreis, im Detailbereich beide) und „Abholung heute möglich bis HH:MM Uhr". Der Kartenpreis **enthält** den Zuschlag — nichts wird addiert. Ein Tarif mit `pickupToday`, aber ohne diese Angaben, bekommt keine Zeile. Das ist der **Normalfall** für die gebührenfreien Abholverträge (TNT, DHL): `pickupToday: true` sagt nur, dass heute abgeholt wird — der Timeline-Knoten heißt „Abholung heute", der Filter greift, und es entsteht trotzdem weder eine Zuschlagszeile noch ein „+ 0,00 €". Die Oberfläche entscheidet darüber nichts: kein Datumsvergleich, keine Uhr, keine ServiceID.
