...
- **Gesperrte Abholung heute** — drei Gründe, sichtbar mit Preis, nicht auswählbar; der Knopf sagt den Satz, darunter immer „Bitte wählen Sie einen späteren Abholtag.":
  ...
  - **Das Versanddatum ist der früheste Abholtag** (TG UPS Effective Collection Date): sperrt allein der Abholtag den Wunschtag, verschiebt der Server kuratierte Services (heute 22, 23 und 26) auf den nächsten Versandtag — das Angebot trägt diesen Tag in `collectionDate` mit „bereit ab 09:00 Uhr" und ist wie jeder spätere Abholtag auswählbar. Die drei Sätze oben erscheinen deshalb nur für Angebote ohne diesen Rückfall. Die Oberfläche entscheidet darüber nicht: kein Datumsvergleich, keine Uhr, keine ServiceID, kein Hinweistext; Karte, ausgewähltes Angebot und Live-Leiste zeigen den Tag des Angebots (`pickupContractOf`, `pickupSummaryOf`), das Versanddatumsfeld bleibt der Wunschtag des Kunden.
...
- **UPS · Standardversand Mehrpaket (freigegeben, TG26):** auswählbar wie der Standardversand — „Vorläufiger Preis", Zuschlagshinweis, Art der Lieferadresse auf der Buchungsseite, danach Absicherung und Buchung, alles über die Verträge oben. Ohne Profil und ohne Prognose bleibt der bisherige Detailbereich: Laufzeit „1–5 Tage", Sendungsverfolgung, Drucker und die Einschränkung „max. 2 Packstücke" kommen aus den Serverfeldern — keine Zahl im Code. Eine Abholung heute gibt es nicht — ein heutiger Wunschtag oder ein Wochenendtag trägt serverseitig den nächsten Versandtag (siehe „Das Versanddatum ist der früheste Abholtag"). Solange die Art der Lieferadresse aussteht, sagt der Server nichts über die Absicherung; der Detailbereich behauptet dann auch kein „Keine Zusatzversicherung verfügbar." (`offerCardInsurance`).
